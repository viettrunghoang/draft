...
from val_data_functions import ValData, ValDataNew
from utils import to_psnr, print_log, validation, adjust_learning_rate
from torchvision.models import vgg16
from perceptual import LossNetwork
from text_loss.text_focus_loss import TextFocusLoss
import os
import numpy as np
import random
...
val_data_loader1 = DataLoader(ValDataNew(val_data_dir,val_filename1, crop_size), batch_size=val_batch_size, shuffle=False, num_workers=8)
...
for epoch in range(epoch_start,num_epochs):
   ...
   # adjust_learning_rate(optimizer, epoch)
    ...
    for batch_id, train_data in enumerate(lbl_train_data_loader):
        ...
        input_image, gt, label = train_data
        ...
        content_loss = text_loss(pred_image, gt, label)
        smooth_loss = F.smooth_l1_loss(pred_image, gt)
        perceptual_loss = loss_network(pred_image, gt)

        loss = smooth_loss + lambda_loss*perceptual_loss + content_loss
    ...
    print("allfilter")
    print_log(epoch+1, num_epochs, one_epoch_time, train_psnr, val_psnr1, val_ssim1, exp_name, loss, content_loss)
